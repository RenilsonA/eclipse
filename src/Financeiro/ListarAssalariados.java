	...
	public void Initialize() {
				...
				if(!ModificacaodeID.getText().isEmpty()) {
					if(Listas.assalariados.size() > 0) {
						...
						for(int a = 0; a < f.length(); a++) {
							...
							int posicao = posic.posicao(0, Integer.parseInt(x));
							if(posicao >= 0) {
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < Listas.assalariados.size(); i++) {
					Object linha[] = new Object[] {Listas.assalariados.get(i).getID(), Listas.assalariados.get(i).getNome(), Listas.assalariados.get(i).getSalario(),
												   Listas.assalariados.get(i).getBonus(), Listas.assalariados.get(i).getGrupo()};
					modelo.addRow(linha);
