	...
	public void Initialize() {
		...
		btnModificarFuncionrios.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if(Listas.comissionados.size() > 0 && !ModificacaodeID.getText().isEmpty()) {
					Listas posic = new Listas();
					...
					for(int a = 0; a < f.length(); a++) {
						...
						int posicao = posic.posicao(2, Integer.parseInt(x));
						if(posicao >= 0) {
			...
			public void actionPerformed(ActionEvent e) {
				if(Listas.comissionados.size() > 0 && !ModificacaodeID.getText().isEmpty()) {
					...
					for(int a = 0; a < f.length(); a++) {
						...
						int posicao = posic.posicao(2, Integer.parseInt(x));
						if(posicao >= 0) {
							CadastrarVenda c = new CadastrarVenda(Listas.comissionados.get(posicao).getID());
		...
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				for (int i = 0; i < Listas.comissionados.size(); i++) {
					Object linha[] = new Object[] {Listas.comissionados.get(i).getID(), Listas.comissionados.get(i).getNome(), Listas.comissionados.get(i).getSalario(),
												   Listas.comissionados.get(i).getVendas(), Listas.comissionados.get(i).getComissao(), Listas.comissionados.get(i).getBonus(),
												   Listas.comissionados.get(i).getGrupo()};
					modelo.addRow(linha);
