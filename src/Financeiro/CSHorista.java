...
public class CSHorista {
	...
	public void Initialize(final int index) {		
		...
		JLabel lblNewLabel = new JLabel("Cadastro Financeiro Horista do ID " + Listas.horistas.get(index).getID());
		...
		comboBox.setSelectedIndex(Listas.horistas.get(index).getGrupo());
		...
		SalarioHora.setText(Float.toString(Listas.horistas.get(index).getSalarioHora()));
		descSind.setEditable(Lista.funcionarios.get(li.posicao(Listas.horistas.get(index).getID())).isSindicato());
		Comissao.setText(Float.toString(Listas.horistas.get(index).getBonus()));
		Alicota.setText(Float.toString(Listas.horistas.get(index).getAlicota()));
		descSind.setText(Float.toString(Listas.horistas.get(index).getDiscExtra()));
		descExt.setText(Float.toString(Listas.horistas.get(index).getDescontos()));
			...
			public void actionPerformed(ActionEvent e) {
				if (JOptionPane.showConfirmDialog(CSHoristas, "Os dados estão corretos?",
					...
					if(comboBox.getSelectedIndex() > 0) {
						Lista.funcionarios.get(li.posicao(Listas.horistas.get(index).getID())).setGrupo(comboBox.getSelectedIndex());
						if(!descSind.getText().isEmpty()) 
							Listas.horistas.get(index).setDiscExtra(Float.parseFloat(descSind.getText()));
						if(!Alicota.getText().isEmpty()) 
							Listas.horistas.get(index).setAlicota(Float.parseFloat(Alicota.getText()));
						if(!descExt.getText().isEmpty()) 
							Listas.horistas.get(index).setDescontos(Float.parseFloat(descExt.getText()));
						...
						Listas.horistas.get(index).setBonus(Extra + extra);
						
						if(!SalarioHora.getText().isEmpty())
							Listas.horistas.get(index).setSalarioHora(Float.parseFloat(SalarioHora.getText()));
						Listas.horistas.get(index).setGrupo(comboBox.getSelectedIndex());
						Tempo t = new Tempo();
						if(comboBox.getSelectedIndex() > 3) Listas.horistas.get(index).setPagamento(Lista.grupos.get(comboBox.getSelectedIndex() - 4).getDia());
						else if(comboBox.getSelectedIndex() == 1) Listas.horistas.get(index).setPagamento(t.UltimoDiaUtil());
						else if(comboBox.getSelectedIndex() == 3) Listas.horistas.get(index).setPagamento(LocalDate.now().plusDays(14 - t.DiaemNumero() + 6));
						else Listas.horistas.get(index).setPagamento(LocalDate.now().plusDays(7 - t.DiaemNumero() + 6));
						Lista l = new Lista();
						Lista.funcionarios.get(l.posicao(Listas.horistas.get(index).getID())).setGrupo(comboBox.getSelectedIndex());
						Listas.horistas.get(index).setGrupo(comboBox.getSelectedIndex());
						Lista.funcionarios.get(l.posicao(Listas.horistas.get(index).getID())).setGrupo(comboBox.getSelectedIndex());
						CSHoristas.dispose();
						try {
