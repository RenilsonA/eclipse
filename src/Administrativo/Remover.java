	...
	public void Initialize() {
		...
		btnCancelar.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				if (JOptionPane.showConfirmDialog(Remocao, "Você realmente quer sair?",
						"Remoçaõ de Funcionários", JOptionPane.YES_NO_OPTION) == JOptionPane.YES_NO_OPTION) {
					Remocao.dispose();
					...
					for(int a = 0; a < f.length(); a++) {
						...
						if(posicao >= 0) {
							if(JOptionPane.showConfirmDialog(Remocao, "Realmente deseja remover o funcionário?\nID: " + x + "\nNome: " + 
									Lista.funcionarios.get(posicao).getNome(), "Remover", 
									 JOptionPane.YES_NO_OPTION) == JOptionPane.YES_NO_OPTION) {
								
								int id = Lista.funcionarios.get(posicao).getID();
								lista.remover(posicao);
								Listas lis = new Listas();
								ListaPonto list = new ListaPonto();
								int assa = lis.posicao(0, id);
								int hori = lis.posicao(1, id);
								int comi = lis.posicao(2, id);
								int banc = lis.posicaoDados(0, id);
								int corr = lis.posicaoDados(1, id);
								if(assa != -1) {
									lis.remover(0, assa);
									list.remover(0, assa);
								} else if (hori != -1) {
								...
								if(banc != -1) {
									lis.removerDados(0, banc);
								} else if(corr != -1) {
									lis.removerDados(1, corr);
								}
							}
						}
					}
